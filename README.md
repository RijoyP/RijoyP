...
Services code: eShopMicroservices
GitOps manifests: GitOps
Terraform modules: terraform-modules
CICD templates: CICD-Templates
Helm boilerplate: helm-templates
...
🔧 Backend Services
Service Overview
ServiceDatabasePurposeKey FeaturesCatalog APIPostgreSQLProduct managementProduct CRUD, categories, inventory, searchBasket APIPostgreSQL + RedisShopping cartCart operations, session management, checkoutDiscount APISQLitePromotionsCoupon validation, discount calculationOrdering APISQL ServerOrder processingDDD implementation, order lifecycle, payments
...
│  Deployment  │
└──────────────┘
Pipeline Files: eShopMicroservices/.pipelines
Automated Profile Generation
Script: generate-profiles.ps1
Purpose: Automatically generates Azure DevOps pipeline YAML files for each microservice across all environments.
How It Works:

Scans the Services directory to discover all microservices
Reads base pipeline templates
Generates service-specific pipeline files for dev, staging, and production
Injects service-specific variables (ports, paths, dependencies)
Validates generated YAML files
Outputs ready-to-use pipeline configurations

Generated Profiles Location: CICD-Templates/applications/profiles
Example Generated Profiles:

catalog-api-dev.yml - Development pipeline for Catalog service
catalog-api-staging.yml - Staging pipeline with integration tests
catalog-api-prod.yml - Production pipeline with approval gates
basket-api-dev.yml - Development pipeline for Basket service
ordering-api-prod.yml - Production pipeline with DDD considerations

Benefits:

Consistency: All services follow the same pipeline structure
Speed: New service gets complete CI/CD in seconds
Maintainability: Update template once, regenerate all profiles
...
Deployment: GitOps/infrastructure/logging
...
Deployment: GitOps/infrastructure/tracing
...
Deployment: GitOps/infrastructure/monitoring
...
Deployment: GitOps/infrastructure/messaging

🔗 Repository Links
RepositoryPurposeeShopMicroservicesMicroservices source codeGitOpsKubernetes manifests and FluxCD configsterraform-modulesInfrastructure as Code modulesCICD-TemplatesReusable pipeline templateshelm-templatesHelm chart boilerplate
...
Domain-Driven Design: Rich domain model in Ordering service with Marten
GitOps: Declarative infrastructure and deployments with FluxCD
